package com.jakob.leetcode.剑指offer.n43;
...
*/
    public int countDigitOne(int n) {
        int num = n, count = 0, factor = 1;
        while (num != 0) {
            int digit = num % 10;
            if (digit == 0) {
                count += (num / 10) * factor;
            } else if (digit == 1) {
                count += (num / 10) * factor + (n % factor) + 1;
            } else {
                count += (num / 10 + 1) * factor;
            }
            num /= 10;
            factor *= 10;
        }
    ...
    public static void main(String[] args) {
        int count = new Solution().countDigitOne(12);
